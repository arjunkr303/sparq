...
const clean = u => {
  ...
  return {
    ...
    isPremium:    !!(u.is_premium && u.premium_expiry && new Date(u.premium_expiry) > now),
...
router.post('/spend-coins', authMw, async (req, res) => {
  try {
    ...
    if (itemId === 'opener') {
      if (!extra?.roomId) {
        return res.status(400).json({ message: 'Active chat room is required for opener' });
      }
      if (!tryClaim(extra.roomId)) {
        return res.status(409).json({ message: 'AI opener already used in this chat' });
      }
    }
