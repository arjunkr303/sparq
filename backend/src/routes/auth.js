...
const clean = u => ({
  ...
  isPremium:    !!(u.is_premium && u.premium_expiry && new Date(u.premium_expiry) > new Date()),
...
router.get('/check-email', async (req, res) => {
  try {
    const { data } = await supabase.from('users').select('id')
      .eq('email', (req.query.email || '').toLowerCase()).maybeSingle();
    res.json({ taken: !!data });
...
router.get('/check-username', async (req, res) => {
  try {
    const { data } = await supabase.from('users').select('id')
      .eq('username', req.query.username || '').maybeSingle();
    res.json({ taken: !!data });
  } catch { res.json({ taken: false }); }
});
...
router.post('/register', async (req, res) => {
  try {
    const { email, password, username, gender, age, country, state, city,
            interests, ageConfirmed } = req.body;

    if (!ageConfirmed)    return res.status(400).json({ message: 'Must confirm 18+' });
    if (parseInt(age) < 18) return res.status(400).json({ message: 'Must be 18+' });
    if (!['male','female','other'].includes(gender))
      return res.status(400).json({ message: 'Invalid gender' });

    const { data: e1 } = await supabase.from('users').select('id')
      .eq('email', email.toLowerCase()).maybeSingle();
    if (e1) return res.status(400).json({ message: 'Email already registered' });

    const { data: e2 } = await supabase.from('users').select('id')
...
router.post('/login', async (req, res) => {
  try {
    ...
    const { data: u, error } = await supabase.from('users').select('*')
      .eq('email', email.toLowerCase()).maybeSingle();
